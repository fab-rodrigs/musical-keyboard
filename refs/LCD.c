...
static inline void pulso_enable(){
    __delay_cycles(1000);
    set_bit(CONTR_LCD,E);
...
{							//o LCD ser� s� escrito. Ent�o, R/W � sempre zero.
	...
	__delay_cycles(100000);   //_delay_ms(100);	 		//tempo para estabilizar a tens�o do LCD, ap�s VCC ultrapassar 4.5 V (na pr�tica pode
							//ser maior). 
	//interface de 8 bits						
	#if (nibble_dados)
	...
	__delay_cycles(100000);   //_delay_ms(5);
	pulso_enable();
	__delay_cycles(100000);   //_delay_us(200);
